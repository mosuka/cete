...
func init() {
	...
	startCmd.PersistentFlags().StringVar(&dataDirectory, "data-directory", "./data", "HTTP server listen address")
	...
	startCmd.PersistentFlags().IntVar(&logMaxSize, "log-max-size", 500, "Max size of a log file (megabytes)")
	startCmd.PersistentFlags().IntVar(&logMaxBackups, "log-max-backups", 3, "Max backup count of log files")
	startCmd.PersistentFlags().IntVar(&logMaxAge, "log-max-age", 30, "Max age of a log file (days)")
