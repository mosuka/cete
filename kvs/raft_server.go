...
func NewRaftServer(node *ceteraft.Node, bootstrap bool, logger *log.Logger) (*RaftServer, error) {
...
func (s *RaftServer) getMetadata(nodeId string) (*ceteraft.Node, error) {
...
func (s *RaftServer) setMetadata(nodeId string, node *ceteraft.Node) error {
	// Node -> Any
	nodeAny := &any.Any{}
...
func (s *RaftServer) Join(node *ceteraft.Node) error {
...
func (s *RaftServer) Leave(node *ceteraft.Node) error {
	if s.raft.State() != raft.Leader {
		// forward to leader node
		leaderId, err := s.LeaderID(60 * time.Second)
		if err != nil {
...
func (s *RaftServer) GetNode() (*ceteraft.Node, error) {
	cf := s.raft.GetConfiguration()
	err := cf.Error()
	if err != nil {
		return nil, err
...
func (s *RaftServer) GetCluster() (*ceteraft.Cluster, error) {
	cf := s.raft.GetConfiguration()
	err := cf.Error()
	if err != nil {
		return nil, err
